...
const getFollowing = (req, res) => {
  const userId = req.user.userId;
  //console.log(userId);
  Connection.query("SELECT u.username , d.writer_id,d.data_content  FROM follows f INNER JOIN users u ON f.writer_id = u.user_id LEFT JOIN data d ON u.user_id = d.writer_id WHERE f.user_id = ?", [userId], (err, data) => {
    if (err) {
      console.log(err);
      return res.status(500).json("Internal Server Error");
    }
