...
const deleteBookmark = (req, res) => {
  ...
  console.log(userId);
  console.log(dataId);
  Connection.query("select count(*) as count from bookmark where data_id=? and user_id =?", [dataId, userId], (err, data) => {
    if (err) {
      console.log(err);
      return res.status(500).json("Internal server error");
    }
    console.log(data);
    if (data[0].count > 0) {
      Connection.query("delete from bookmark where data_id=? and user_id=?", [dataId, userId], (err, data) => {
        if (err) {
          console.log(err);
          return res.status(500).json("Internal server error");
        }
